...
void poseCallback(const nav_msgs::Odometry::ConstPtr& msg) {
    ...
    ROS_INFO("Se la posicion del robot en x = [%.3f] y = [%.3f]", odom_msg.pose.pose.position.x,odom_msg.pose.pose.position.y);
}
...
int main(int argc, char **argv) {
    ...
    if (!n.getParam("lado_cuadrado", lado_cuadrado)) {
        ROS_WARN("Parametro /lado_cuadrado no encontrado. Usando valor por defecto: 5");
        lado_cuadrado = 5;
    } else {
        ROS_INFO("Parametro /lado_cuadrado obtenido: %d", lado_cuadrado);
    }
    ...
    ros::Subscriber odom_sub = n.subscribe("/odom", 2, poseCallback);
    ...
    while (ros::ok() )  {
        ...
        cmd_vel_msg.linear.x = 0.0;  // Velocidad lineal en el eje X (m/s).
